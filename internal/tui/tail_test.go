...
func TestModel_TailEventMsg_AdjustsScrollOffset(t *testing.T) {
	m := makeTailModel()
	...
	// scrollOffset should have been reduced by trimmed count
	if m.tailScrollOffset != 0 {
		t.Errorf("expected scrollOffset 0 (5 - 10 clamped), got %d", m.tailScrollOffset)
	}
}
